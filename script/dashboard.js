const socket = io("https://one337-chat-server.onrender.com/", {
...
fetch(`https://one337-chat-server.onrender.com/user/alreadyConnectedUser?userId=${userId}`)
...
function renderConnectedUsers(data) {
    ...
    data.forEach((user) => {
        ...
        li.addEventListener("click", () => {
            fetch(
                `https://one337-chat-server.onrender.com/user/getAllMessages?user1=${userId}&user2=${user._id}`
            )
                .then((res) => res.json())
                .then((res) => {
...
searchBtn.addEventListener("click", () => {
    ...
    fetch(
        `https://one337-chat-server.onrender.com/user/searchUser?search=${search}&userId=${userId}`,
        {
            method: "GET",
            headers: {
...
function renderUsers(users) {
    ...
    users.forEach((el) => {
        ...
        div.addEventListener("click", () => {
            ...
            fetch(
                `https://one337-chat-server.onrender.com/user/getAllMessages?user1=${userId}&user2=${el._id}`
            )
                .then((res) => res.json())
                .then((res) => {
                    if (res.length == 0) {
...
function createGroup() {
    const url = new URLSearchParams(window.location.search);
    const userId = url.get("id");
    fetch(`https://one337-chat-server.onrender.com/user/allUser?userId=${userId}`)
        .then((res) => res.json())
        .then((response) => {
